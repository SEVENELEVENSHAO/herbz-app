...
export function RandomPlantIcon() {
  useEffect(() => {
    const previous = sessionStorage.getItem("herbz-icon");
    ...
    sessionStorage.setItem("herbz-icon", emoji);
    document.documentElement.dataset.herbzIcon = emoji;
    window.dispatchEvent(new CustomEvent("herbz-icon-change", { detail: emoji }));
